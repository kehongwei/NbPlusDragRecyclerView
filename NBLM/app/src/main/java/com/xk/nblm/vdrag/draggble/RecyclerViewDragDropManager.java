...
package com.xk.nblm.vdrag.draggble;
...
import com.xk.nblm.vdrag.adapter.AdapterPath ;
import com.xk.nblm.vdrag.adapter.ItemIdComposer ;
import com.xk.nblm.vdrag.utils.CustomRecyclerViewUtils ;
...
//            this.layoutType = CustomRecyclerViewUtils.getLayoutType(rv);
            this.layoutType = CustomRecyclerViewUtils.LAYOUT_TYPE_GRID_VERTICAL;
            this.vertical = true;
//            this.vertical = CustomRecyclerViewUtils.extractOrientation(this.layoutType) == CustomRecyclerViewUtils.ORIENTATION_VERTICAL;
    ...
    private int mItemSettleBackIntoPlaceAnimationDuration = 1000;
    ...
    private int mItemMoveMode = ITEM_MOVE_MODE_SWAP;
    ...
    /*package*/ boolean onInterceptTouchEvent(RecyclerView rv, MotionEvent e) {
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_CANCEL:
                ...
                handled = handleActionUpOrCancel(action, true,false);
                break;
    ...
    /*package*/ void onTouchEvent(RecyclerView rv, MotionEvent e) {
        ...
        switch (action) {
            case MotionEvent.ACTION_UP:
            case MotionEvent.ACTION_CANCEL:
                Log.i("khw","=====onTouchEvent抬起=====");
                handleActionUpOrCancel(action, true, true);
                break;
...
//        Log.i("khw","=====按下时===坐标== "+e.getX()+"  "+e.getY());
        ...
        boolean handled;
        Log.i("khw","=====按下时=====长按状态 "+mInitiateOnLongPress);
    ...
    private void startDragging(RecyclerView rv, MotionEvent e, RecyclerView.ViewHolder holder, ItemDraggableRange range, AdapterPath path, int wrappedItemPosition, Object composedAdapterTag) {
        ...
        Log.i("khw","=====开始拖拽===== "+path.lastSegment().tag);
...
//        final int layoutType = CustomRecyclerViewUtils.getLayoutType(mRecyclerView);
        final int layoutType = CustomRecyclerViewUtils.LAYOUT_TYPE_GRID_VERTICAL;
    ...
    /*package*/ void cancelDrag(boolean immediately) {
        handleActionUpOrCancel(MotionEvent.ACTION_CANCEL, false, false);
    ...
    private void finishDragging(boolean result) {
        ...
        if (mWrapperAdapter != null) {
            ...
            Log.i("khw","====哪里出错了==== "+draggingItemInitialPosition+"  "+ draggingItemCurrentPosition+" "+result);
...
//        }
    ...
    private boolean handleActionUpOrCancel(int action, boolean invokeFinish, boolean isOnTouch) {
        ...
        mCanDragH = false;
        mCanDragV = false;
        Log.i("khw","====重置状态==== "+invokeFinish+"   "+isDragging());
    ...
    private boolean checkConditionAndStartDragging(RecyclerView rv, MotionEvent e, boolean checkTouchSlop) {
        ...
        Log.i("khw","=====长按位置===== "+wrappedItemPosition);
...
//        Log.i("khw","====拖拽ing=== "+getLastTouchX()+"  "+getLastTouchY());
...
//            checkItemSwapping(rv);
    ...
    void checkItemSwapping(RecyclerView rv) {
        ...
        Log.i("khw","====检查是否需要交换位置==== "+draggingItemInitialPosition+"  "+draggingItemCurrentPosition);

        swapTarget = findSwapTargetItem(mTempSwapTarget, fc, false);
        Log.i("khw","====交换后的位置==== "+swapTarget.position);
...
//        final boolean swapNextItemSmoothlyInLinearLayout = isLinearLayout && (!mCheckCanDrop);
        final boolean isLinearLayout = false ;
        final boolean swapNextItemSmoothlyInLinearLayout = false ;
...
//        final boolean isVertical = (CustomRecyclerViewUtils.extractOrientation(layoutType) == CustomRecyclerViewUtils.ORIENTATION_VERTICAL);
        ...
        Log.i("khw","=====swap====="+firstVisible+"   "+rootFromPosition+"  "+rootToPosition+"  "+fromPosition+"  "+toPosition);
    ...
    private boolean checkTouchedItemState(RecyclerView rv, RecyclerView.ViewHolder holder) {
        ...
        Log.i("khw","=====检查触摸位置===== "+wrappedItemPosition+"  "+adapter.getItemCount());
